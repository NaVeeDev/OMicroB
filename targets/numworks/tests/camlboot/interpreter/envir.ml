...
let env_set_variant_type key cs env =
  { env with variant_types = SMap.add key (true, cs) env.variant_types }
...
let env_extend exported env1 data =
  ...
  { values = merge env1.values (values data.mod_values);
    ...
    variant_types = merge env1.variant_types data.mod_variant_types;
...
let make_module_data env =
           ...
           | Instance_variable _ -> assert false) in
  {
    ...
    mod_variant_types = exported env.variant_types;
...
let prevent_export env =
  ...
  { values = prevent env.values;
    ...
    variant_types = prevent env.variant_types;
...
and env_get_variant_type env ({ loc; _ } as lid) =
  let env_name, env, id = decompose env_get_module_data env lid in
  lookup "polymorphic variant type" ~env_name
    env.variant_types { txt = id; loc }
