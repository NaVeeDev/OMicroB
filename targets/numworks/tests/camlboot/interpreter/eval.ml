...
and eval_expr prims env expr =
  ...
  | Pexp_open (open_decl, e) ->
    let mod_expr = open_decl.popen_expr in
    let m = eval_module_expr prims env mod_expr in
    let nenv = env_extend true env (get_module_data open_decl.popen_loc m) in
    eval_expr prims nenv e
  | Pexp_pack me -> ptr @@ ModVal (eval_module_expr prims env me)
  | Pexp_object _ -> unsupported expr.pexp_loc; assert false
  | Pexp_letop _ -> unsupported expr.pexp_loc; assert false
...
and pattern_bind prims init_env pat v =
  (* We distinguish two environments while we perform pattern-matching:
     - the "pattern environment", the environment in which
       variant constructor names and record fields are looked up
     - the "result environment", the environment containing the
       variables captured by the pattern

     Pattern constructs binding new variables will grow the result environment.
     On the other hand, the "local open" construct Foo.(p) will grow
     the pattern environment.
     *)
  let rec bind ~pat_env ~res_env pat v =
    ...
    | Ppat_any -> res_env
    | Ppat_var s -> env_set_value s.txt v res_env
    | Ppat_alias (p, s) -> env_set_value s.txt v (bind ~pat_env p v ~res_env)
    | Ppat_constant c ->
      if value_equal (value_of_constant c) v then res_env else raise Match_fail
    | Ppat_interval (c1, c2) ->
    ...
    | Ppat_tuple pl ->
      ...
      | Tuple vl ->
        assert (List.length pl = List.length vl);
        List.fold_left2 (fun res_env p v ->
          bind ~pat_env ~res_env p v
        ) res_env pl vl
      | _ -> mismatch pat.ppat_loc; assert false)
    | Ppat_construct (c, p) ->
      let cn = lident_name c.txt in
      let dn = env_get_constr pat_env c in
      (match Ptr.get v with
      | Constructor (ccn, ddn, e) ->
        ...
        (match (p, e) with
        | None, None -> res_env
        | Some p, Some e -> bind ~pat_env ~res_env p e
        | _ -> mismatch pat.ppat_loc; assert false)
      | String s ->
        ...
        let fmt_ebb_of_string =
          ...
          in
          match env_get_value_or_lvar pat_env { loc = c.loc; txt = lid } with
    ...
    | Ppat_record (rp, _) ->
      ...
      | Record r ->
        List.fold_left
          (fun res_env (lident, p) ->
            bind ~pat_env ~res_env p !(SMap.find (lident_name lident.txt) r))
          res_env
          rp
      | _ -> mismatch pat.ppat_loc; assert false)
    | Ppat_array ps ->
       (match Ptr.get v with
          ...
          | _ -> mismatch pat.ppat_loc; assert false)
    | Ppat_or (p1, p2) ->
      (try bind ~pat_env ~res_env p1 v
       with Match_fail -> bind ~pat_env ~res_env p2 v)
    | Ppat_constraint (p, _) -> bind ~pat_env ~res_env p v
    | Ppat_type lident ->
      let variants = env_get_variant_type pat_env lident in
      (match Ptr.get v with
       | Poly_variant (name, _arg) ->
         if not (List.mem name variants) then raise Match_fail;
         res_env
       | _ -> mismatch pat.ppat_loc; assert false)
    | Ppat_lazy _ -> unsupported pat.ppat_loc; assert false
    | Ppat_unpack name ->
      (match Ptr.get v with
      | ModVal m -> env_set_module name.txt m res_env
      | _ -> mismatch pat.ppat_loc; assert false)
    | Ppat_exception _ -> raise Match_fail
    | Ppat_open (lident, p) ->
      let pat_env =
        let module_data = env_get_module_data pat_env lident in
        env_extend false pat_env module_data
      in
      bind ~pat_env ~res_env p v
    | Ppat_extension _ -> unsupported pat.ppat_loc; assert false
  in
  bind ~pat_env:init_env pat v ~res_env:init_env
...
and eval_class_expr prims env class_expr =
  ...
  *)
  ...
  | Pcl_open (open_descr, cle) ->
    let open_decl =
      { open_descr with
        popen_expr = {
          pmod_desc = Pmod_ident open_descr.popen_expr;
          pmod_loc = open_descr.popen_loc;
          pmod_attributes = [];
        };
      }
    in
    eval_as_exp (Pexp_open (open_decl, new_ cle))
  | Pcl_extension _ ->
...
and eval_structitem prims env it =
  ...
  | Pstr_type (_, tl) ->
    List.fold_left eval_type_decl env tl
  | Pstr_typext { ptyext_constructors = ext_constrs; _ } ->
    List.fold_left eval_extension_constructor env ext_constrs
  | Pstr_exception { ptyexn_constructor = ext_constr; _ } ->
    eval_extension_constructor env ext_constr
  ...
  | Pstr_modtype _ -> env
  | Pstr_open { popen_expr = mod_expr; popen_loc } ->
    let m = eval_module_expr prims env mod_expr in
    env_extend false env (get_module_data popen_loc m)
...
and eval_type_decl env t =
  ...
  | Ptype_abstract ->
    (* Consider the declarations
       ...
       We must remember that u is a polymorphic variant
       with constructors `A, `B, `C to be able to interpret

         match ... with
         | #u -> ...
         | `D -> ...
    *)
    let rec collect_polymorphic_variants core_type =
      match core_type.ptyp_desc with
      | Ptyp_variant (fields, _closed_flag, _labels) ->
        Some (
          let names_of_field { prf_desc = field; _ } =
            match field with
            | Rtag ({ txt = label }, _constant, _types) ->
              [ label ]
            | Rinherit ty ->
              begin match collect_polymorphic_variants ty with
              | None -> assert false
              | Some labels -> labels
              end
          in
          fields
          |> List.map names_of_field
          |> List.flatten
        )
      | Ptyp_constr (lident, _args) ->
        begin match env_get_variant_type env lident with
          | vs -> Some vs
          | exception Not_found -> None
        end
      | _ -> None
    in
    begin match t.ptype_manifest with
      | Some ({ ptyp_desc = Ptyp_variant _ } as ty) ->
        begin match collect_polymorphic_variants ty with
        | None -> env
        | Some variants ->
          env_set_variant_type t.ptype_name.txt variants env
        end
      | Some _ -> env
      | None -> env
    end
...
and eval_extension_constructor env { pext_name = name; pext_kind = k; _ } =
  match k with
  | Pext_decl _ ->
    let d = next_exn_id () in
    env_set_constr name.txt d env
